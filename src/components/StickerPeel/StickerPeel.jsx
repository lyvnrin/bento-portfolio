...
const StickerPeel = ({
  ...
  const handleClick = () => {
    if (isPeeling) return
    if (peelBackActivePct < peelThreshold) return

    setIsPeeling(true)

    gsap.to(rootRef.current, {
      '--sticker-peelback-active': '100%',
      duration: 0.5,
      ease: peelEasing,
  ...
  const cssVars = useMemo(
    () => ({
      ...
      // The peel geometry (clip-path/flap) is authored for a top-anchored
      // peel; rotating the container by peelDirection inverts the fold
      // direction, so geometry always stays unrotated. peelDirection is
      // still used above to steer the specular light position.
      '--peel-direction': '0deg',
    }),
    [
      rotate,
  ...
  return (
    <div
      className={`sticker-peel${isPeeling ? ' is-peeling' : ''} ${className}`}
      ref={rootRef}
      style={cssVars}
      onClick={handleClick}
      role="button"
